...
# 来源 PR: #391 (https://github.com/agentic-develop-playground/backlog/pull/391)
# 合入 Issue: #395 - 订阅会议链接功能
...
@pytest.mark.p0
def test_meeting_sig_email_list_field_exists(auth_token):
    """TC-API-001 [正常流] 会议列表API返回sig_email_list字段
    维度: 正常流 | 优先级: P0 | 来源: #395 | PR: #391
    """
...
@pytest.mark.p0
def test_meeting_sig_email_list_with_valid_email_list(auth_token):
...
@pytest.mark.p1
def test_meeting_sig_email_list_without_email_list(auth_token):
    """TC-API-003 [正常流] 无邮件列表时sig_email_list字段为null
    维度: 正常流 | 优先级: P1 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
...
@pytest.mark.p1
def test_meeting_batch_query_no_n_plus_one(auth_token):
    """TC-API-006 [性能] 批量查询避免N+1问题
    维度: 性能 | 优先级: P2 | 来源: #395 | PR: #391
...
@pytest.mark.p1
def test_meeting_same_group_consistent_email_list(auth_token):
    """TC-API-007 [集成] 同一SIG组多个会议的订阅链接一致
    维度: 集成 | 优先级: P1 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
    params = {"date": "2026-05-28"}
    headers = {"Authorization": f"Bearer {auth_token}"}
...
@pytest.mark.p1
def test_meeting_different_groups_distinct_email_list(auth_token):
...
@pytest.mark.p1
def test_meeting_query_failure_graceful_degradation(auth_token):
    """TC-API-009 [异常] 查询失败时sig_email_list为null且不影响主流程
    维度: 异常 | 优先级: P1 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
...
@pytest.mark.p2
def test_meeting_empty_group_name_list(auth_token):
    """TC-API-010 [边界值] 空group_name列表时正常处理
    维度: 边界值 | 优先级: P2 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
...
@pytest.mark.p1
def test_meeting_no_auth_returns_401():
    """TC-API-011 [权限] 无Token访问会议列表API返回401
    维度: 权限 | 优先级: P1 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
    params = {"date": "2026-05-28"}
...
@pytest.mark.p2
def test_meeting_invalid_date_format(auth_token):
    """TC-API-012 [异常输入] 无效日期格式时API正常处理
    维度: 异常输入 | 优先级: P2 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
...
@pytest.mark.p1
def test_meeting_null_group_name(auth_token):
    """TC-API-013 [边界值] group_name参数为null时正常处理
    维度: 边界值 | 优先级: P1 | 来源: #395 | PR: #391
    """
    url = f"{BASE_API}/ascend-meeting/meeting/"
...
def test_meeting_special_char_group_name(auth_token, special_char):
...
@pytest.mark.p2
def test_meeting_long_group_name(auth_token):
...
# |--------|------|----------|--------|--------|
# | TC-API-001 | 会议列表API返回sig_email_list字段 | TASK1 | P0 | #391 |
# | TC-API-002 | 有邮件列表时sig_email_list字段有值 | TASK1 | P0 | #391 |
# | TC-API-003 | 无邮件列表时sig_email_list字段为null | TASK1 | P1 | #391 |
# | TC-API-004 | 权限平台数据源启用时正确查询邮件列表（需后端单元测试） | TASK1 | P1 | #391 |
# | TC-API-005 | 权限平台数据源禁用时使用本地数据库查询（需后端单元测试） | TASK1 | P1 | #391 |
# | TC-API-006 | 批量查询避免N+1问题 | TASK1 | P2 | #391 |
# | TC-API-007 | 同一SIG组多个会议的订阅链接一致 | TASK1 | P1 | #391 |
# | TC-API-008 | 不同SIG组的订阅链接正确区分 | TASK1 | P1 | #391 |
# | TC-API-009 | 查询失败时sig_email_list为null且不影响主流程 | TASK1 | P1 | #391 |
# | TC-API-010 | 空group_name列表时正常处理 | TASK1 | P2 | #391 |
# | TC-API-011 | 无Token访问会议列表API返回401 | - | P1 | #391 |
# | TC-API-012 | 无效日期格式时API正常处理 | - | P2 | #391 |
# | TC-API-013 | group_name参数为null时正常处理 | TASK1 | P1 | #391 |
# | TC-API-014 | group_name包含特殊字符时安全处理 | TASK1 | P1 | #391 |
# | TC-API-015 | 超长group_name时API正常处理 | TASK1 | P2 | #391 |
# | TC-API-016 | 权限平台服务宕机时自动降级（需后端单元测试或Chaos测试） | TASK1 | P1 | #391 |
# | TC-API-017 | 数据库连接失败时优雅降级（需后端单元测试） | TASK1 | P1 | #391 |
# | TC-API-018 | 权限平台返回异常数据格式时容错处理（需后端单元测试） | TASK1 | P2 | #391 |
# | TC-UI-001 | 有邮件列表时显示订阅链接和帮助图标（人工测试） | TASK2 | P0 | #391 |
# | TC-UI-002 | 无邮件列表时显示灰色提示文案（人工测试） | TASK2 | P0 | #391 |
# | TC-UI-003 | 订阅链接跳转到正确的邮件订阅页面（人工测试） | TASK2 | P1 | #391 |
