# -*- coding: utf-8 -*-
"""
unifiedbus 集成测试脚本：OneID 登录 + 会议中心全量接口
=================================================================

数据来源：
    本文件用例集基于 meeting/meeting.jmx 的请求结构与字段设计而成，
    域名/账号体系迁移到 unifiedbus 社区（id.unifiedbus / unifiedbus）。

被测接口清单（unifiedbus 社区）：
    [ OneID ]
    1. GET  https://id.unifiedbus.test.osinfra.cn/oneid/public/key
    2. POST https://id.unifiedbus.test.osinfra.cn/oneid/login
    [ 会议中心 ]
    3. GET    https://unifiedbus.test.osinfra.cn/api-meeting/group_info/
    4. GET    https://unifiedbus.test.osinfra.cn/api-meeting/platform/
    5. POST   https://unifiedbus.test.osinfra.cn/api-meeting/
    6. PUT    https://unifiedbus.test.osinfra.cn/api-meeting/{id}/
    7. DELETE https://unifiedbus.test.osinfra.cn/api-meeting/{id}/
...
EXPECTED_USERNAME =  "unifiedbus_ewk6aomh"  # 实测正常登录返回的 username

PASSWORD = os.environ.get("TEST_PASSWORD")
DEFAULT_TIMEOUT = 10

# unifiedbus OneID 登录头：Origin/Referer 指向用户中心根路径，
# 沿用同一套 OneID 鉴权协议
COMMON_HEADERS = {
...
def test_tc_api_login_001_normal_flow():
    ...
    # 不同社区账号注册方式不一：邮箱注册账号 phone_exist=False，
    # 手机号注册账号 email_exist=False；此处仅断言「至少有一个标识可用」。
    assert data.get("email_exist") is True or data.get("phone_exist") is True, \
        f"账号既无邮箱也无手机号绑定：{data}"
...
def test_tc_api_login_004_wrong_client_id():
    ...
    模块：OneID/登录 | 优先级：P1
    unifiedbus 域 login body 不带 redirect_uri，client_id 非法预期返回 4xx 业务码失败。
    """
...
# 三、会议业务常量与工具函数
# =========================================================
DEFAULT_GROUP = os.environ.get("DEFAULT_GROUP", "品牌营销组")
DEFAULT_PLATFORM = os.environ.get("DEFAULT_PLATFORM", "WELINK")
# etherpad 域名因社区而异；unifiedbus 默认沿用通用 etherpad，可通过环境变量覆盖
DEFAULT_ETHERPAD = os.environ.get(
    "DEFAULT_ETHERPAD",
    "https://etherpad.unifiedbus.test.osinfra.cn/p/group-brandMarketing",
)

PATH_MEETING = "/api-meeting/"
PATH_MEETING_DETAIL = "/api-meeting/{meeting_id}/"
PATH_MEETING_GROUP_INFO = "/api-meeting/group_info/"
PATH_MEETING_PLATFORM = "/api-meeting/platform/"
