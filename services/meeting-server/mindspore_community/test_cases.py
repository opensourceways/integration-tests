# -*- coding: utf-8 -*-
"""
mindspore 集成测试脚本：OneID 登录 + 会议中心全量接口
=================================================================

数据来源：
    本文件用例集基于 meeting/meeting.jmx 的请求结构与字段设计而成，
    域名/账号体系迁移到 mindspore 社区（mindspore-usercenter / mindspore-website）。

被测接口清单（mindspore 社区）：
    [ OneID ]
    1. GET  https://mindspore-usercenter.test.osinfra.cn/oneid/public/key
    2. POST https://mindspore-usercenter.test.osinfra.cn/oneid/login
    [ 会议中心 ]
    3. GET    https://mindspore-website.test.osinfra.cn/api-meeting/v1/meeting/group_info/
    4. GET    https://mindspore-website.test.osinfra.cn/api-meeting/v1/meeting/platform/
    5. POST   https://mindspore-website.test.osinfra.cn/api-meeting/v1/meeting/
    6. PUT    https://mindspore-website.test.osinfra.cn/api-meeting/v1/meeting/{id}/
    7. DELETE https://mindspore-website.test.osinfra.cn/api-meeting/v1/meeting/{id}/
...
# =========================================================
# 域名来源：mindspore 社区
#   userURL=mindspore-usercenter.test.osinfra.cn
#   hostURL=mindspore-website.test.osinfra.cn
USER_URL = os.environ.get("USER_URL", "mindspore-usercenter.test.osinfra.cn")
HOST_URL = os.environ.get("HOST_URL", "mindspore-website.test.osinfra.cn")
BASE_AUTH = f"https://{USER_URL}"
BASE_BIZ = f"https://{HOST_URL}"

ACCOUNT = os.environ.get("TEST_ACCOUNT")
CLIENT_ID = "678e12840a894118623d1f2c"
EXPECTED_USERNAME =  "guoxiaozhen0304"  # 实测正常登录返回的 username

PASSWORD = os.environ.get("TEST_PASSWORD")
DEFAULT_TIMEOUT = 10

# mindspore OneID 登录头：Origin/Referer 指向用户中心根路径，
# 沿用同一套 OneID 鉴权协议
COMMON_HEADERS = {
...
def test_tc_api_login_001_normal_flow():
    ...
    预期结果：
        ...
        4. body.data.username = "guoxiaozhen0304"
...
def test_tc_api_login_004_wrong_client_id():
    ...
    模块：OneID/登录 | 优先级：P1
    mindspore 域 login body 不带 redirect_uri，client_id 非法预期返回 4xx 业务码失败。
    """
...
# 三、会议业务常量与工具函数
# =========================================================
DEFAULT_GROUP = os.environ.get("DEFAULT_GROUP", "test")
DEFAULT_PLATFORM = os.environ.get("DEFAULT_PLATFORM", "WELINK")
# etherpad 域名因社区而异；mindspore 默认沿用通用 etherpad，可通过环境变量覆盖
DEFAULT_ETHERPAD = os.environ.get(
    "DEFAULT_ETHERPAD", "https://etherpad.mindspore.cn/p/test"
)

PATH_MEETING = "/api-meeting/v1/meeting/"
PATH_MEETING_DETAIL = "/api-meeting/v1/meeting/{meeting_id}/"
PATH_MEETING_GROUP_INFO = "/api-meeting/v1/meeting/group_info/"
PATH_MEETING_PLATFORM = "/api-meeting/v1/meeting/platform/"
...
#
# 人工执行步骤:
#   1. 浏览器打开 https://mindspore-usercenter.test.osinfra.cn/login
