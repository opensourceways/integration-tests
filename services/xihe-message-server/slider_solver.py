...
# 对外接口
# =============================================================================
def _wait_images_ready(box, timeout_ms: int = 8000) -> bool:
    """
    等待弹窗内底图与拼图块两张图解码完成。

    组件先渲染容器再异步填 base64，刚弹出时 querySelectorAll('img') 可能返回空，
    直接分析会白白消耗一次尝试机会，故先轮询等图就绪。
    ...
    while time.time() < deadline:
        try:
            ready = box.evaluate("""e => {
                const imgs = [...e.querySelectorAll('img')]
                    .filter(i => i.complete && i.naturalWidth > 0
                                 && String(i.src).startsWith('data:'));
...
def solve_slider(page, max_attempts: int = 3) -> bool:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            # ---- 等两张图真正解码完成 ----
            # 弹窗容器先于图片渲染，若立刻读取会拿到 0 张图而白白耗掉一次尝试机会
            if not _wait_images_ready(box):
                raise SliderSolveError("等待底图/拼图块加载超时（图片未解码完成）")
