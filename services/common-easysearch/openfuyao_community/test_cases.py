# -*- coding: utf-8 -*-
"""
API-common 接口自动化测试用例 - openfuyao 社区
...
class TestSearchSig:
    """POST /search/sig - SIG 组检索"""

    def test_sig_search_positive(self, client: RequestClient):
        case_id = "SEARCH-SIG-001"
        logger.info(f"[{case_id}] 开始执行: SIG 组检索")
        group = AssertionGroup(case_id=case_id)
        body = {"keyword": "kernel", "lang": "zh", "page": 1, "pageSize": 10}
        resp = client.post("/search/sig", json_data=body)
        assert_http_status(resp.http_status, 200, group)
        assert_business_status(resp.business_status, 200, group)
        assert_not_none(resp.data, name="sig_data", group=group)
        logger.info(group.summary())
        assert group.all_passed, f"用例 {case_id} 存在断言失败"
